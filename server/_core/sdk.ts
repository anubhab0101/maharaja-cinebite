...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    if (ENV.oAuthServerUrl) {
      ...
      console.log("[OAuth] Using Google OAuth 2.0 & Staff Auth (OAUTH_SERVER_URL optional)");
    }
  }
...
class SDKServer {
  ...
  private getSessionSecret() {
    ...
    if (!secret || secret.trim().length === 0) {
      if (ENV.isProduction || process.env.NODE_ENV === "production") {
        throw new Error("FATAL SECURITY ERROR: JWT_SECRET environment variable must be configured in production!");
      }
      if (!devFallbackSecret) {
        devFallbackSecret = crypto.randomBytes(32).toString("hex");
        console.warn("[Security Warning] JWT_SECRET is not configured! Generated temporary cryptographically strong in-memory secret for this session. Set JWT_SECRET in .env for production.");
      }
  ...
  async createSessionToken(
    ...
    return this.signSession(
      {
        openId,
        appId: ENV.appId || "cinebites",
        name: options.name || "Staff Member",
        email: options.email,
        role: options.role,
      },
      options
  ...
  async verifySession(
    ...
    try {
      ...
      return {
        openId,
        appId: isNonEmptyString(appId) ? appId : (ENV.appId || "cinebites"),
        name: isNonEmptyString(name) ? name : "Staff Member",
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // If user not in DB, sync from OAuth server automatically or use session fallback
    if (!user) {
      if (ENV.oAuthServerUrl) {
        try {
          const userInfo = await this.getUserInfoWithJwt(sessionToken ?? "");
