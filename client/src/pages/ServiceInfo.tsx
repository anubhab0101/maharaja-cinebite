...
> = {
  ...
  "/cookies": {
    ...
    paragraphs: [
      "The installable staff web app uses a service worker to store a public offline notice and its stylesheet on your device. It does not store order/API responses or authenticated pages in that offline cache. Staff can separately enable foreground sound and background order notifications. Lock-screen text contains no customer name, phone or seat. Background setup requires server configuration and device permission; delivery is not guaranteed.",
      ...
      "Offer enrollment may be disabled until setup is complete. Customer iPhone/iPad web push is not offered because it requires Home Screen installation. Staff may install the app and separately enable order alerts. Staff subscriptions store a user ID, push endpoint, encryption keys and enrollment/expiry times for up to seven days unless renewed or stopped. Delivery retry records expire after one day through worker cleanup. Access is checked against the current staff directory; stopped/expired/revoked subscriptions are excluded. Signing out alone does not stop a subscribed device; use Stop background alerts on shared phones. Push travels through Google, Mozilla or Apple services. Notification arrival or loudness is not guaranteed: browser, network, device volume and Do Not Disturb settings apply. Spoken alerts work only while the page is open.",
